# -*- coding: utf-8 -*-
"""
Created on Mon Jun 03 18:38:29 2013
...
class UserBasedCF:
    ...
    def recommend(self, user, train=None, K=8, nitem=40):
        ...
        for v, wuv in sorted(self.userSimilarityBest[user].items(), key=lambda x : x[1], reverse=True)[0:K]:
    ...
    def recallAndPrecison(self, train=None, test=None, K=8, nitem=10):
        train = train or self.traindata
    ...
    def popularity(self, train=None, test=None, K=8, nitem=10):
        ...
        for user in train.keys():
            rank = self.recommend(user, train=train, K=K, nitem=nitem)
            for item,_ in rank.items():
                ret += math.log( 1 + item.popularity[item] )
                n += 1
        return ret / (n * 1.0)
